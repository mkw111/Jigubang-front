...
const EnergyDetailPage: React.FC = () => {
    ...
    const neighborUsage = compareData?.neighborAverage || 0;
    ...
    const pctDiff = neighborUsage > 0 ? parseFloat((((neighborUsage - myUsage) / neighborUsage) * 100).toFixed(1)) : 0;
    ...
    return (
        <div className="page-container insight-wrapper">
            ...
            <main className="app-content insight-content">
                ...
                {/* Tab Contents */}
                {myUsage <= 0 ? (
                    <div className="card tab-card animated-fade-in" style={{ padding: '40px 20px', textAlign: 'center', color: 'var(--color-text-muted)', margin: '12px 0' }}>
                        <span style={{ fontSize: '48px', display: 'block', marginBottom: '16px' }}>⚡</span>
                        <p style={{ fontWeight: 600, fontSize: '15px', color: 'var(--color-text-dark)', margin: '0 0 8px 0' }}>조회된 에너지 데이터가 없습니다.</p>
                        <p style={{ fontSize: '13px', margin: 0 }}>원격검침 장비가 미등록 상태이거나, 실거주 인증 승인 완료 후에 데이터 확인이 가능합니다.</p>
                    </div>
                ) : (
                    <>
                        {activeTab === 'daily' && (
